import{S,U as E}from"./main-6c126296.js";import"./apply_course-23113af5.js";import{S as H}from"./swiper-core-19160206.js";import{N as M,A}from"./autoplay-054b67c1.js";import{l as x}from"./index-00c34100.js";import{F as b}from"./favorites-a706b415.js";import{C as v}from"./comment-a44fa049.js";import{P as j}from"./passes-fbcc49a6.js";import{C as I}from"./course-0f83fb1f.js";import"https://cdn.jsdelivr.net/npm/aos@2.3.4/+esm";import"https://unpkg.com/@wangeditor/editor@latest/dist/index.js";import"./avatars-cf792582.js";const B=document.querySelector(".course-title section"),f=document.querySelector(".favorite-btn"),P=f.querySelector(".course-favorite-icon"),U=document.querySelector(".blackboard-mark"),D=document.querySelector(".course-cover"),F=document.querySelector(".course-scores"),O=document.querySelector(".course-tags"),i=document.querySelector(".course-test"),h=document.querySelector(".classroom-btn"),q=document.querySelector(".test-passed"),V=document.querySelector(".course-page-nav"),y=V.querySelectorAll("[data-key]"),m=document.querySelector(".comment-list"),g=document.querySelector(".pagination"),N=document.querySelector(".other-courses > ul"),$=new URLSearchParams(window.location.search),l=$.has("id")&&$.get("id");let d=$.has("sort")&&$.get("sort"),p,n,r;async function w(){if(!localStorage.getItem("token")){f.classList.add("d-none"),h.classList.add("d-none");return}try{if(r=await E.getUserInfo(),!r)return;f.classList.remove("d-none"),r.favorites.find(e=>Number(e.courseId)===Number(l))?P.classList.remove("outline-icon"):P.classList.add("outline-icon"),r.passes.some(e=>Number(e.courseId)===Number(l))&&C()}catch{f.classList.add("d-none"),h.classList.add("d-none")}}async function R(){try{x(),await w(),p=await I.getCourse(l),n=await v.getComments(1,6,d,"desc",l),x(p),_(p),d==="timer"&&y[0].classList.add("active"),d==="likesNum"&&y[1].classList.add("active"),L(n),T(),z()}catch{}}function C(){i.classList.add("d-none"),q.classList.remove("d-none"),h.classList.remove("d-none"),h.setAttribute("href",`classroom.html?courseId=${l}`)}function _(e){B.innerHTML=`
  ...
  </li>`}).join("")}function T(){if(!n.totalPages){g.innerHTML="";return}let e="";for(let s=1;s<=n.totalPages;s++)e+=`<li class="page-item p-hover ${n.currentPage===s?"active":""}"><a class="page-link" href="#" data-link="${s}">${s}</a></li>`;g.innerHTML=`
  ...
  <li class="page-item p-hover">
  ...
  </li>`,g.addEventListener("click",t);async function t(s){s.preventDefault(),s.stopPropagation(),s.target.dataset.link&&(g.removeEventListener("click",t),s.target.dataset.link&&(n=await v.getComments(Number(s.target.dataset.link),6,d,"desc",l)),L(n),T())}}async function z(){const e=await I.getAllCourses(),t=[];e.forEach(a=>{a.tags.some(u=>p.tags.includes(u))&&t.push(a)});const s=t.filter(a=>a.avgScore>=3);if(!s.length){N.innerHTML=`
    ...
    `).join(""),new H(".other-courses",{slidesPerView:1,spaceBetween:30,centeredSlides:!1,loop:!0,breakpoints:{992:{slidesPerView:3}},modules:[M,A],autoplay:{delay:3e3,disableOnInteraction:!1},navigation:{nextEl:".swiper-next",prevEl:".swiper-prev"}}),document.querySelectorAll(".others-favorite-btn").forEach(a=>{a.addEventListener("click",async c=>{c.preventDefault();const u=r.favorites.findIndex(k=>Number(k.courseId)===Number(c.target.dataset.id));if(u===-1)await b.add(l),a.classList.remove("outline-icon");else{const k=r.favorites[u].id;await b.remove(k),a.classList.add("outline-icon")}await w()})})}R();f.addEventListener("click",async e=>{e.stopPropagation();const t=r.favorites.findIndex(s=>Number(s.courseId)===Number(l));if(t===-1)await b.add(l);else{const s=r.favorites[t].id;await b.remove(s)}await w()});m.addEventListener("click",async e=>{if(!e.target.dataset.comment)return;const t=n.data.find(u=>u.id===Number(e.target.dataset.comment)),{id:s,likes:o,likesNum:a}=t;if(o.includes(r.id))return;o.push(r.id);const c={likes:o,likesNum:o.length};v.likeComment(s,c),L(await v.getComments(Number(e.target.dataset.link),6,d,"desc",s))});i.addEventListener("submit",e=>{if(e.preventDefault(),i["test-name"].classList.remove("is-invalid"),i["test-brand"].classList.remove("is-invalid"),i["test-name"].value.trim()||i["test-name"].classList.add("is-invalid"),i["test-brand"].value.trim()||i["test-brand"].classList.add("is-invalid"),![...i.querySelectorAll(".is-invalid")].length){if(i["test-name"].value.trim()!==p.title||i["test-brand"].value.trim()!==p.platform){i.reset(),S.fire({scrollbarPadding:!1,icon:"error",title:"回答錯誤,再找找答案吧"});return}if(!localStorage.getItem("token")){S.fire({scrollbarPadding:!1,icon:"error",title:"請加入會員",showConfirmButton:!1,timer:1500});return}j.addPasses({userId:r.id,courseId:Number(l),isFinish:!1}),C()}});y.forEach(e=>{e.addEventListener("click",async t=>{y.forEach(o=>o.classList.remove("active")),e.classList.add("active"),d=t.target.dataset.key,m.classList.add("justify-content-around"),m.innerHTML=`<li class="spinner-border m-0 text-success" role="status" style="width: 100px; height: 100px;">
    <span class="visually-hidden">Loading...</span>
  </li>`;const s=await v.getComments(1,6,d,"desc",l);L(s)})});
