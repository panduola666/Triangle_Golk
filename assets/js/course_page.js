...
function renderCourse(course) {
  ...
  <span class="bg-secondary text-white py-2 px-4 rounded-1 me-lg-3 me-2 mb-1 mb-lg-0">${course.platform}</span>
...
function renderComment(comments) {
  ...
  // 有評論內容
  commentList.innerHTML = comments.data
    .map((comment, index) => {
    ...
    <article
      class="fw-bold ${
        comment.theme ? 'course-page-comment' : 'comment-style0'
      } bg-${
        (index + 1) % 3 === 0
          ? 'blue'
          : (index + 1) % 3 === 1
          ? 'primary'
          : 'orange'
      } px-3 pb-3 d-flex flex-column h-100"
    >
    <div class="flex-grow-1">
        <div class="stars-icon d-flex float-end">
          <span class="material-symbols-outlined ${
            comment.score >= 1 ? '' : 'outline-icon'
          }">star</span>
          <span class="material-symbols-outlined ${
            comment.score >= 2 ? '' : 'outline-icon'
          }">star</span>
          <span class="material-symbols-outlined ${
            comment.score >= 3 ? '' : 'outline-icon'
          }">star</span>
          <span class="material-symbols-outlined ${
            comment.score >= 4 ? '' : 'outline-icon'
          }">star</span>
          <span class="material-symbols-outlined ${
            comment.score >= 5 ? '' : 'outline-icon'
          }">star</span>
        </div>
        <p class="fs-5 mb-2">${
          Number(comment.showName) ? comment.user.nickName : hiddenName
        }：</p>
        <div class="fs-6">
          ${comment.content}
        </div>
      ...
      <div class="${
        localStorage.getItem('token') ? 'd-flex' : 'd-none'
      } align-items-center gap-2">
          <!-- 用 .active 控制是否點擊 -->
        <span class="thumb-up ${
          user && comment.likes.includes(user.id) ? 'active' : ''
        } material-symbols-outlined cur-point" data-comment="${comment.id}">
          thumb_up
          </span>
...
function renderPagination() {
  ...
  for (let page = 1; page <= comments.totalPages; page++) {
    str += `<li class="page-item p-hover ${
      comments.currentPage === page ? 'active' : ''
    }"><a class="page-link" href="#" data-link="${page}">${page}</a></li>`;
  }
  pagination.innerHTML = `
  <li class="page-item p-hover">
      <a class="page-link ${
        comments.currentPage === 1 ? 'd-none' : ''
      }" href="#" aria-label="Previous" data-link="${comments.currentPage - 1}">
  ...
  <li class="page-item p-hover">
      <a class="page-link ${
        comments.currentPage === comments.totalPages ? 'd-none' : ''
      }" href="#" aria-label="Next" data-link="${comments.currentPage + 1}">
...
async function renderOthers() {
  ...
  otherCourses.innerHTML = final
    .map((item) => {
      return `
    <div class="swiper-slide">
      <li class="d-flex flex-column">
        <a class="card card-hover h-100 rounded-4 position-relative" href="./coursepages.html?id=${
          item.id
        }&sort=timer">
          <div class="pic">
            <img
          ...
          >
            ...
            } favorite others-favorite-btn material-symbols-outlined position-absolute ${
        localStorage.getItem('token') &&
        item.favorites.some((userLike) => userLike.userId === user.id)
          ? ''
          : 'outline-icon'
      }"
            data-id="${item.id}"
          ...
          <div class="card-body">
            <h3 class="title text-secondary fs-6 fw-bold text-truncate">${
              item.title
            }</h3>
          </div>
          <div class="card-footer">
            ...
            ${item.tags
              .map((tag) => `<span class="fs-8 me-2">#${tag}</span>`)
              .join('')}
            </div>
            <div class="stars d-flex">
              <div class="stars-icon text-primary me-2">
                <span class="material-symbols-outlined ${
                  item.avgScore >= 1 ? '' : 'outline-icon'
                }">star</span>
                <span class="material-symbols-outlined ${
                  item.avgScore >= 2 ? '' : 'outline-icon'
                }">star</span>
                <span class="material-symbols-outlined ${
                  item.avgScore >= 3 ? '' : 'outline-icon'
                }">star</span>
                <span class="material-symbols-outlined ${
                  item.avgScore >= 4 ? '' : 'outline-icon'
                }">star</span>
                <span class="material-symbols-outlined ${
                  item.avgScore >= 5 ? '' : 'outline-icon'
                }"
                  >star</span
                >
              </div>
  ...
  otherBtn.forEach((btn) => {
    btn.addEventListener('click', async (e) => {
      ...
      const index = user.favorites.findIndex(
        (item) => Number(item.courseId) === Number(e.target.dataset.id)
      );
      if (index === -1) {
        // 加入
